...
---Does nothing
function main.replicate(tbl, imp) end
...
function main.extend(ts, td, force)
  spruce.validate({ "table", "table", { "nil", "boolean" } }, { ts, td, force })
...
function main.contains(tbl, val)
  spruce.validate({ "table", "some" }, { tbl, val })
...
function main.boolean(tbl, recurse)
  tbl, recurse = table.unpack(
    spruce.parse_args({ "table", { "boolean", "nil", def = false } }, { tbl, recurse })
  )
...
function main.map(tbl, fun, track)
  ...
  spruce.validate({ "table", "function", "boolean" }, { tbl, fun, track })
  local rvl = {}
  for k, v in pairs(tbl) do
